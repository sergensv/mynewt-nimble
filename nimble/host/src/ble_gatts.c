...
static int ble_gatts_svc_access(uint16_t conn_handle, uint16_t attr_handle, uint8_t op, uint16_t offset, struct os_mbuf **om, void *arg)
{
  const struct ble_gatt_svc_def *svc;
  uint8_t *buf;
...
static int ble_gatts_inc_access(uint16_t conn_handle, uint16_t attr_handle, uint8_t op, uint16_t offset, struct os_mbuf **om, void *arg)
{
...
static uint16_t ble_gatts_chr_clt_cfg_allowed(const struct ble_gatt_chr_def *chr)
{
  ...
  if( chr->flags & BLE_GATT_CHR_F_NOTIFY   ) { flags |= BLE_GATTS_CLT_CFG_F_NOTIFY  ; }
  if( chr->flags & BLE_GATT_CHR_F_INDICATE ) { flags |= BLE_GATTS_CLT_CFG_F_INDICATE; }
...
static uint8_t ble_gatts_att_flags_from_chr_flags(ble_gatt_chr_flags chr_flags)
{
  ...
  if( chr_flags & BLE_GATT_CHR_F_READ                                  ) { att_flags |= BLE_ATT_F_READ        ; }
  if( chr_flags & (BLE_GATT_CHR_F_WRITE_NO_RSP | BLE_GATT_CHR_F_WRITE) ) { att_flags |= BLE_ATT_F_WRITE       ; }
  if( chr_flags & BLE_GATT_CHR_F_READ_ENC                              ) { att_flags |= BLE_ATT_F_READ_ENC    ; }
  if( chr_flags & BLE_GATT_CHR_F_READ_AUTHEN                           ) { att_flags |= BLE_ATT_F_READ_AUTHEN ; }
  if( chr_flags & BLE_GATT_CHR_F_READ_AUTHOR                           ) { att_flags |= BLE_ATT_F_READ_AUTHOR ; }
  if( chr_flags & BLE_GATT_CHR_F_WRITE_ENC                             ) { att_flags |= BLE_ATT_F_WRITE_ENC   ; }
  if( chr_flags & BLE_GATT_CHR_F_WRITE_AUTHEN                          ) { att_flags |= BLE_ATT_F_WRITE_AUTHEN; }
  if( chr_flags & BLE_GATT_CHR_F_WRITE_AUTHOR                          ) { att_flags |= BLE_ATT_F_WRITE_AUTHOR; }
...
static uint8_t ble_gatts_chr_properties(const struct ble_gatt_chr_def *chr)
{
  ...
  if( chr->flags & BLE_GATT_CHR_F_BROADCAST                                   ) { properties |= BLE_GATT_CHR_PROP_BROADCAST      ; }
  if( chr->flags & BLE_GATT_CHR_F_READ                                        ) { properties |= BLE_GATT_CHR_PROP_READ           ; }
  if( chr->flags & BLE_GATT_CHR_F_WRITE_NO_RSP                                ) { properties |= BLE_GATT_CHR_PROP_WRITE_NO_RSP   ; }
  if( chr->flags & BLE_GATT_CHR_F_WRITE                                       ) { properties |= BLE_GATT_CHR_PROP_WRITE          ; }
  if( chr->flags & BLE_GATT_CHR_F_NOTIFY                                      ) { properties |= BLE_GATT_CHR_PROP_NOTIFY         ; }
  if( chr->flags & BLE_GATT_CHR_F_INDICATE                                    ) { properties |= BLE_GATT_CHR_PROP_INDICATE       ; }
  if( chr->flags & BLE_GATT_CHR_F_AUTH_SIGN_WRITE                             ) { properties |= BLE_GATT_CHR_PROP_AUTH_SIGN_WRITE; }
  if( chr->flags & (BLE_GATT_CHR_F_RELIABLE_WRITE | BLE_GATT_CHR_F_AUX_WRITE) ) { properties |= BLE_GATT_CHR_PROP_EXTENDED       ; }
...
static int ble_gatts_chr_def_access(uint16_t conn_handle, uint16_t attr_handle, uint8_t op, uint16_t offset, struct os_mbuf **om, void *arg)
{
...
static uint8_t ble_gatts_chr_op(uint8_t att_op)
{
  switch (att_op) 
    {
      case BLE_ATT_ACCESS_OP_READ : return BLE_GATT_ACCESS_OP_READ_CHR;
      case BLE_ATT_ACCESS_OP_WRITE: return BLE_GATT_ACCESS_OP_WRITE_CHR;

      default:  BLE_HS_DBG_ASSERT(0);
...
static void ble_gatts_chr_inc_val_stat(uint8_t gatt_op)
{
  switch (gatt_op) 
    {
      case BLE_GATT_ACCESS_OP_READ_CHR  : STATS_INC(ble_gatts_stats, chr_val_reads ); break;
      case BLE_GATT_ACCESS_OP_WRITE_CHR : STATS_INC(ble_gatts_stats, chr_val_writes); break;
...
static int ble_gatts_val_access(uint16_t conn_handle, uint16_t attr_handle, uint16_t offset, struct ble_gatt_access_ctxt *gatt_ctxt, struct os_mbuf **om, ble_gatt_access_fn *access_cb, void *cb_arg)
{
...
static int ble_gatts_chr_val_access(uint16_t conn_handle, uint16_t attr_handle, uint8_t att_op, uint16_t offset, struct os_mbuf **om, void *arg)
{
  ...
  rc = ble_gatts_val_access(conn_handle, attr_handle, offset, &gatt_ctxt, om, chr_def->access_cb, chr_def->arg);
...
static int ble_gatts_register_inc(struct ble_gatts_svc_entry *entry)
{
  ...
  rc = ble_att_svr_register(uuid_inc, BLE_ATT_F_READ, 0, &handle, ble_gatts_inc_access, entry);
...
static uint8_t ble_gatts_dsc_op(uint8_t att_op)
{
  switch (att_op) 
    {
      case BLE_ATT_ACCESS_OP_READ   : return BLE_GATT_ACCESS_OP_READ_DSC;
      case BLE_ATT_ACCESS_OP_WRITE  : return BLE_GATT_ACCESS_OP_WRITE_DSC;
...
static void ble_gatts_dsc_inc_stat(uint8_t gatt_op)
{
  switch (gatt_op) 
    {
      case BLE_GATT_ACCESS_OP_READ_DSC  : STATS_INC(ble_gatts_stats, dsc_reads ); break;
      case BLE_GATT_ACCESS_OP_WRITE_DSC : STATS_INC(ble_gatts_stats, dsc_writes); break;
...
static int ble_gatts_dsc_access(uint16_t conn_handle, uint16_t attr_handle, uint8_t att_op, uint16_t offset, struct os_mbuf **om, void *arg)
{
  ...
  rc = ble_gatts_val_access(conn_handle, attr_handle, offset, &gatt_ctxt, om, dsc_def->access_cb, dsc_def->arg);
...
static int ble_gatts_register_dsc(const struct ble_gatt_svc_def *svc, const struct ble_gatt_chr_def *chr, const struct ble_gatt_dsc_def *dsc, uint16_t chr_def_handle, ble_gatt_register_fn *register_cb, void *cb_arg)
{
  ...
  int rc;
    ...
    rc = ble_att_svr_register(dsc->uuid, dsc->att_flags, dsc->min_key_size, &dsc_handle, ble_gatts_dsc_access, (void *)dsc);
...
static void ble_gatts_subscribe_event(uint16_t conn_handle, uint16_t attr_handle, uint8_t reason, uint8_t prev_flags, uint8_t cur_flags)
{
  if( (prev_flags ^ cur_flags) & ~BLE_GATTS_CLT_CFG_F_RESERVED ) 
    {
      ble_gap_subscribe_event(conn_handle, attr_handle, reason, prev_flags  & BLE_GATTS_CLT_CFG_F_NOTIFY,
...
static int ble_gatts_clt_cfg_access_locked(struct ble_hs_conn *conn, uint16_t attr_handle, uint8_t att_op, uint16_t offset, struct os_mbuf *om, struct ble_store_value_cccd *out_cccd, uint8_t *out_prev_clt_cfg_flags, uint8_t *out_cur_clt_cfg_flags)
{
  ...
  /* We always register the client characteristics descriptor with handle  (chr_val + 1).  */
                                              ...
                                              if( conn->bhc_sec_state.bonded ) 
                                                {
                                                  ...
                                                  out_cccd->peer_addr.type = ble_hs_misc_peer_addr_type_to_id(conn->bhc_peer_addr.type);
...
int ble_gatts_clt_cfg_access(uint16_t conn_handle, uint16_t attr_handle, uint8_t op, uint16_t offset, struct os_mbuf **om, void *arg)
{
  ...
  if( conn == NULL ) 
    ...
    {
      rc = ble_gatts_clt_cfg_access_locked(conn, attr_handle, op, offset, *om, &cccd_value, &prev_flags, &cur_flags);
    }
  ...
  ble_gatts_subscribe_event(conn_handle, chr_val_handle, BLE_GAP_SUBSCRIBE_REASON_WRITE, prev_flags, cur_flags);
...
static int ble_gatts_register_clt_cfg_dsc(uint16_t *att_handle)
{
  ...
  rc = ble_att_svr_register(uuid_ccc, BLE_ATT_F_READ | BLE_ATT_F_WRITE, 0, att_handle, ble_gatts_clt_cfg_access, NULL);
...
static int ble_gatts_register_chr(const struct ble_gatt_svc_def *svc, const struct ble_gatt_chr_def *chr, ble_gatt_register_fn *register_cb, void *cb_arg)
{
  struct ble_gatt_register_ctxt register_ctxt;
  ...
  /* Register characteristic definition attribute (cast away const on
    * callback arg).
    */
  rc = ble_att_svr_register(uuid_chr, BLE_ATT_F_READ, 0, &def_handle, ble_gatts_chr_def_access, (void *)chr);
  ...
  rc = ble_att_svr_register(chr->uuid, att_flags, chr->min_key_size, &val_handle, ble_gatts_chr_val_access, (void *)chr);
...
static int  ble_gatts_svc_type_to_uuid(uint8_t svc_type, const ble_uuid_t **uuid)
{
  switch (svc_type) 
    {
      case BLE_GATT_SVC_TYPE_PRIMARY    : *uuid = uuid_pri;  return 0;
      case BLE_GATT_SVC_TYPE_SECONDARY  : *uuid = uuid_sec;  return 0;
...
{
  if( svc->type != BLE_GATT_SVC_TYPE_PRIMARY && svc->type != BLE_GATT_SVC_TYPE_SECONDARY ) 
    {  return 0;  }
...
static int ble_gatts_register_svc(const struct ble_gatt_svc_def *svc, uint16_t *out_handle, ble_gatt_register_fn *register_cb, void *cb_arg)
{
  const struct ble_gatt_chr_def *chr;
  struct ble_gatt_register_ctxt register_ctxt;
  ...
  /* Register service definition attribute (cast away const on callback
    * arg).
    */
  rc = ble_att_svr_register(uuid, BLE_ATT_F_READ, 0, out_handle, ble_gatts_svc_access, (void *)svc);
  ...
  if( clt_cfgs != NULL ) 
    {
      for( i = 0; i < num_clt_cfgs; i++ ) 
        {
          ble_gatts_subscribe_event(conn_handle, clt_cfgs[i].chr_val_handle, BLE_GAP_SUBSCRIBE_REASON_TERM, clt_cfgs[i].flags, 0);
        }
...
int ble_gatts_start(void)
{
  ...
  for (i = 0; i < ble_gatts_num_svc_defs; i++) 
    {
      rc = ble_gatts_register_svcs(ble_gatts_svc_defs[i], ble_hs_cfg.gatts_register_cb, ble_hs_cfg.gatts_register_arg);
  ...
  rc = os_mempool_init(&ble_gatts_clt_cfg_pool, num_elems, ble_gatts_clt_cfg_size(), ble_gatts_clt_cfg_mem, "ble_gatts_clt_cfg_pool");
...
{
  return ble_gatts_num_cfgable_chrs == 0 || ble_gatts_clt_cfg_pool.mp_num_free > 0;
}
...
static uint8_t ble_gatts_schedule_update(struct ble_hs_conn *conn, struct ble_gatts_clt_cfg *clt_cfg)
{
  ...
  if( !(clt_cfg->flags & BLE_GATTS_CLT_CFG_F_MODIFIED)) 
    ...
    {
      /* Peer isn't subscribed to notifications or indications.  Nothing to send. */
      att_op = 0;
    }

  /* If we will be sending an update, clear the modified flag so that we don't double-send. */
  if( att_op != 0 ) {
  ...
  if( conn->bhc_gatt_svr.indicate_val_handle == chr_val_handle) 
    {
      ...
      persist = conn->bhc_sec_state.bonded && !(clt_cfg->flags & BLE_GATTS_CLT_CFG_F_MODIFIED);
    ...
    {
      /* This acknowledgement doesn't correspond to the outstanding indication; ignore it. */
      rc = BLE_HS_ENOENT;
    }
...
void ble_gatts_chr_updated(uint16_t chr_val_handle)
{
  ...
  for( i = 0; ; i++ ) 
    {
      /* XXX: This is inefficient when there are a lot of connections. Consider using a "foreach" function to walk the connection list. */
...
static void ble_gatts_tx_notifications_one_chr(uint16_t chr_val_handle)
{
  ...
  /* Determine if notifications / indications are enabled for this characteristic. */
  ...
  for (i = 0; ; i++) 
    {
      ...
      if( conn == NULL ) 
        {
          break;             /* No more connected devices. */
        }

      switch (att_op) 
        {
          case 0                      :  break;
          case BLE_ATT_OP_NOTIFY_REQ  :  ble_gatts_notify  (conn_handle, chr_val_handle); break;
          case BLE_ATT_OP_INDICATE_REQ:  ble_gatts_indicate(conn_handle, chr_val_handle); break;
  ...
  while( 1) 
    {
      ...
      clt_cfg = ble_gatts_clt_cfg_find(conn->bhc_gatt_svr.clt_cfgs, cccd_value.chr_val_handle);
      ...
      /* Tell the application if the peer changed its subscription state
        ...
        */
      ble_gatts_subscribe_event(conn_handle, cccd_value.chr_val_handle, BLE_GAP_SUBSCRIBE_REASON_RESTORE, 0, cccd_value.flags);

      switch (att_op) 
        {
          case 0                      : break;

          case BLE_ATT_OP_NOTIFY_REQ  : rc = ble_gatts_notify(conn_handle, cccd_value.chr_val_handle);
          ...
          case BLE_ATT_OP_INDICATE_REQ: ble_gatts_indicate(conn_handle, cccd_value.chr_val_handle);
...
static int ble_gatts_find_svc_chr_attr(const ble_uuid_t *svc_uuid, const ble_uuid_t *chr_uuid, struct ble_gatts_svc_entry **out_svc_entry, struct ble_att_svr_entry **out_att_chr)
{
  ...
  while( 1) 
    {
      ...
      if( ble_uuid_u16(cur->ha_uuid) == BLE_ATT_UUID_CHARACTERISTIC && next != NULL && ble_uuid_cmp(next->ha_uuid, chr_uuid) == 0 ) 
        {
          if( out_svc_entry != NULL ) {  *out_svc_entry = svc_entry;  }
          if( out_att_chr   != NULL ) {  *out_att_chr   = next;       }
...
int ble_gatts_find_chr(const ble_uuid_t *svc_uuid, const ble_uuid_t *chr_uuid, uint16_t *out_def_handle, uint16_t *out_val_handle)
{
  ...
  if( out_def_handle) { *out_def_handle = att_chr->ha_handle_id - 1; }
  if( out_val_handle) { *out_val_handle = att_chr->ha_handle_id    ; }
...
int ble_gatts_find_dsc(const ble_uuid_t *svc_uuid, const ble_uuid_t *chr_uuid, const ble_uuid_t *dsc_uuid, uint16_t *out_handle)
{
  struct ble_gatts_svc_entry *svc_entry;
  struct ble_att_svr_entry *att_chr;
  struct ble_att_svr_entry *cur;
  ...
  for (i = 0; i < ble_gatts_num_svc_entries; i++) 
    {
      ...
      if( entry->handle == handle) 
        {
          if( visible) {  ble_att_svr_restore_range(entry->handle, entry->end_group_handle);  } 
          else         {  ble_att_svr_hide_range   (entry->handle, entry->end_group_handle);  }
...
static int ble_gatts_count_resources(const struct ble_gatt_svc_def *svcs, struct ble_gatt_resources *res)
{
  const struct ble_gatt_svc_def *svc;
  const struct ble_gatt_chr_def *chr;
  int s;
          ...
          for( c = 0; svc->characteristics[c].uuid != NULL; c++ ) 
            {
              ...
              /* If the characteristic permits notifications or indications, it has a CCCD. */
              if( chr->flags & BLE_GATT_CHR_F_NOTIFY || chr->flags & BLE_GATT_CHR_F_INDICATE) 
                {
                  /* Each CCCD requires:
                    *     o 1 descriptor
                    *     o 1 CCCD
...
void ble_gatts_lcl_svc_foreach(ble_gatt_svc_foreach_fn cb, void *arg)
{
  ...
  for (i = 0; i < ble_gatts_num_svc_entries; i++) 
    {
      cb(ble_gatts_svc_entries[i].svc, ble_gatts_svc_entries[i].handle, ble_gatts_svc_entries[i].end_group_handle, arg);
    }
}
...
int ble_gatts_init(void)
{
  ...
  rc = stats_init_and_reg( STATS_HDR(ble_gatts_stats), STATS_SIZE_INIT_PARMS(ble_gatts_stats, STATS_SIZE_32), STATS_NAME_INIT_PARMS(ble_gatts_stats), "ble_gatts");
